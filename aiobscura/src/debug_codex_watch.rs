//! aiobscura-debug-codex-watch - Incremental Codex CLI parser watcher
//!
//! Watches Codex CLI log files for changes and outputs new content incrementally.
//! Uses polling to reliably detect append operations on macOS.
//! Outputs JSON in the same format as debug_codex for composability.

use aiobscura_core::ingest::parsers::CodexParser;
...
#[derive(Parser)]
#[command(name = "aiobscura-debug-codex-watch")]
#[command(about = "Watch Codex CLI logs and output new content incrementally")]
#[command(version)]
struct Args {
    ...
    /// Recursive directory scan (for nested date directories)
    #[arg(long, short)]
    recursive: bool,
}
...
fn main() -> Result<()> {
    ...
    let files: Vec<PathBuf> = if args.path.is_file() {
        ...
        collect_jsonl_recursive(&args.path)?
    } else {
    ...
    eprintln!("Use debug_codex for existing content. Ctrl+C to stop...");
    ...
    loop {
        ...
        if args.path.is_dir() {
            let current_files = if args.recursive {
                collect_jsonl_recursive(&args.path).unwrap_or_default()
            } else {
                std::fs::read_dir(&args.path)
                    .ok()
                    .map(|entries| {
...
fn collect_jsonl_recursive(dir: &Path) -> Result<Vec<PathBuf>> {
    let mut files = Vec::new();
    collect_jsonl_recursive_inner(dir, &mut files)?;
    Ok(files)
}

fn collect_jsonl_recursive_inner(dir: &Path, files: &mut Vec<PathBuf>) -> Result<()> {
    if !dir.is_dir() {
        return Ok(());
    }
    ...
    for entry in std::fs::read_dir(dir)? {
        ...
        if path.is_dir() {
            collect_jsonl_recursive_inner(&path, files)?;
