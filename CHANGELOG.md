...
- Upgraded workspace dependencies to latest available releases across all crates.
- Refreshed lockfile to latest compatible patch versions.
- Updated `reqwest` TLS feature configuration for the 0.13 series.

### Fixed

- Adjusted SQLite PRAGMA size reads for `rusqlite 0.38` type constraints with safe conversion.
- Removed stale UI import after `ratatui 0.30` upgrade.
